...
if (!function_exists('db_get_all_data')) {
    function db_get_all_data($table_name = null, $where = false)
    {
        ...
        // Periksa apakah $query adalah objek sebelum memanggil result()
        if (is_object($query)) {
            return $query->result();
        } else {
            // Log error atau kembalikan array kosong
            log_message('error', 'Query failed for table: ' . $table_name);
            return array();
        }
    }
}
...
if (!function_exists('display_menu_module')) {
	function display_menu_module($role, $parent, $level, $menu_type, $ignore_active = false)
	{
		...
		// $ssql = "SELECT a.id, a.label, a.type, a.active, a.link, Deriv1.Count FROM `menu` a  
		// 		LEFT OUTER JOIN (
		// 			SELECT parent, COUNT(*) AS Count FROM `menu` GROUP BY parent
		// 		) Deriv1 ON a.id = Deriv1.parent 
		// 		WHERE a.menu_type_id = " . $menu_type_id . " AND a.parent=" . $parent . " " . ($ignore_active ? '' : 'and active = 1') . ($role != 1 ? 'and a.access = 0' : '') .  "  order by `sort` ASC";
		
		// $result = $ci->db->query($ssql)->result();

		$result = $ci->db->query("
			SELECT a.id, a.label, a.type, a.active, a.link, Deriv1.Count 
			FROM menu a
			LEFT OUTER JOIN (
			...
			WHERE a.menu_type_id = " . $menu_type_id . " 
			AND a.parent = " . $parent . " " 
			. ($ignore_active ? '' : 'AND active = 1') 
			. ($role != 1 ? ' AND a.access = 0' : '') 
			. " ORDER BY \"sort\" ASC"
		)->result();
...
if (!function_exists('display_menu_admin')) {
	function display_menu_admin($role, $parent, $level, $menu_type_id = false)
	{
		...
		// Tambahkan sebelum return $result
		// file_put_contents(APPPATH . 'logs/menu_debug.txt', 
