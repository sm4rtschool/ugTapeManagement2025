...
class Aauth
{
	...
	public function create_user($role, $email, $pass, $username = FALSE, $optional_data = [])
	{
		...
		$query = $this->aauth_db->insert($this->config_vars['users'], $data);
	...
	public function get_user($user_id = FALSE)
	{
		...
		// file_put_contents(APPPATH . 'logs/get_user.txt', 
		// 	'QUERY: ' . $this->aauth_db->last_query() . "\n\n" . 
		// 	'RESULT: ' . print_r($query->row(), true), FILE_APPEND);
	...
	public function create_group($group_name, $definition = '')
	{
		...
		if ($query->num_rows() < 1) {
			...
			$query2 = $this->db->query("SELECT currval(pg_get_serial_sequence('" . $this->config_vars['groups'] . "','id')) AS last_id");
	...
	public function create_perm($perm_name, $definition = '')
	{
		...
		if ($query->num_rows() < 1) {
			...
			$query2 = $this->aauth_db->query("SELECT currval(pg_get_serial_sequence('" . $this->config_vars['perms'] . "','id')) AS last_id");
			// echo $this->aauth_db->last_query(); inez
			// exit;
