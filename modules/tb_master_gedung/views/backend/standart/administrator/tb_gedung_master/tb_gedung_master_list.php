<script type="text/javascript">
   function domo() {
      ...
      $('*').bind('keydown', 'Ctrl+a', function() {
         window.location.href = ADMIN_BASE_URL + '/tb_master_gedung/add';
         return false;
      });
...
<!-- <section class="content-header">
   <h1>
      <?= cclang('Master Data Gedung') ?><small><?= cclang('list_all'); ?></small>
   </h1>
   <ol class="breadcrumb">
      <li><a href="#"><i class="fa fa-dashboard"></i> Home</a></li>
      <li class="active"><?= cclang('Master Data Gedung') ?></li>
   </ol>
</section> -->
<!-- Main content -->
               ...
               <div class="box box-widget widget-user-2">
                  <div class="widget-user-header ">
                     <div class="row pull-right">
                        <?php is_allowed('tb_master_gedung_add', function () { ?>
                           <a class="btn btn-flat btn-success btn_add_new" id="btn_add_new" title="<?= cclang('add_new_button', [cclang('tb_master_gedung')]); ?>  (Ctrl+a)" href="<?= admin_site_url('/tb_master_gedung/add'); ?>"><i class="fa fa-plus-square-o"></i> Add Gedung</a>
                        <?php }) ?>
                        <?php is_allowed('tb_master_gedung_export', function () { ?>
                           <a class="btn btn-flat btn-success" title="<?= cclang('export'); ?> <?= cclang('tb_master_gedung') ?> " href="<?= admin_site_url('/tb_master_gedung/export?q=' . $this->input->get('q') . '&f=' . $this->input->get('f')); ?>"><i class="fa fa-file-excel-o"></i> <?= cclang('export'); ?> XLS</a>
                        <?php }) ?>
                        <?php is_allowed('tb_master_gedung_export', function () { ?>
                           <a class="btn btn-flat btn-success" title="<?= cclang('export'); ?> pdf <?= cclang('tb_master_gedung') ?> " href="<?= admin_site_url('/tb_master_gedung/export_pdf?q=' . $this->input->get('q') . '&f=' . $this->input->get('f')); ?>"><i class="fa fa-file-pdf-o"></i> <?= cclang('export'); ?> PDF</a>
                        <?php }) ?>
                     </div>
                     ...
                     <!-- /.widget-user-image -->
                     <h3 class="widget-user-username"><?= cclang('Master Data Gedung') ?></h3>
                     <h5 class="widget-user-desc"><?= cclang('list_all', [cclang('Master Data Gedung')]); ?> <i class="label bg-yellow"><span class="total-rows"><?= $tb_gedung_master_counts; ?></span> <?= cclang('items'); ?></i></h5>
                  </div>

                  <form name="form_tb_master_gedung" id="form_tb_master_gedung" action="<?= admin_base_url('/tb_master_gedung/index'); ?>">
                     ...
                     <div class="row">
                        <div class="col-md-8">
                           ...
                           <div class="col-sm-1 padd-left-0 ">
                              <a class="btn btn-default btn-flat" name="reset" id="reset" value="Apply" href="<?= admin_base_url('/tb_master_gedung'); ?>" title="<?= cclang('reset_filter'); ?>">
                        ...
                        <table id="exampleas" class="table table-bordered table-striped dataTable">
                           <thead>
                              <tr class="">
                                 ...
                                 <th data-field="id" data-primary-key="0"> Kode Gedung</th>
                                 <th data-field="kode_gedung" data-primary-key="0"> Nama Gedung</th>
                                 <th data-field="ket_gedung" data-primary-key="0"> Keterangan Gedung</th>
                                 <th data-field="nama_area" data-primary-key="0"> Nama Area</th>
                                 <th>Action</th>
                              </tr>
                           </thead>
                           <tbody id="tbody_tb_master_gedung">
                              <?= $tables ?>
                           </tbody>
                        </table>
                     </div>
...
<script>
   var module_name = "tb_master_gedung";
...
<script>
   $(document).ready(function() {
      ...
      //    var bulk = $('#bulk');
      //    var serialize_bulk = $('#form_tb_master_gedung').serialize();
      ...
      //          function(isConfirm) {
      //             if (isConfirm) {
      //                document.location.href = ADMIN_BASE_URL + '/tb_master_gedung/delete?' + serialize_bulk;
      //             }
      //          });
      ...
      // });
      // initSortableAjax('tb_master_gedung', $('table.dataTable'));
   }); /*end doc ready*/
</script>