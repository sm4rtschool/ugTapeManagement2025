<script type="text/javascript">
   function domo() {
      ...
      $('*').bind('keydown', 'Ctrl+a', function() {
         window.location.href = ADMIN_BASE_URL + '/tb_master_pegawai/add';
         return false;
      });
...
<section class="content-header">
   <h1>
      <?= cclang('Master Pegawai') ?><small><?= cclang('list_all'); ?></small>
   </h1>
   <ol class="breadcrumb">
      <li><a href="#"><i class="fa fa-dashboard"></i> Home</a></li>
      <li class="active"><?= cclang('Master Pegawai') ?></li>
   </ol>
</section>
<!-- Main content -->
                  ...
                  <div class="widget-user-header ">
                     <div class="row pull-right">
                        <?php is_allowed('tb_pegawai_master_add', function () { ?>
                           <a class="btn btn-flat btn-success btn_add_new" id="btn_add_new" title="<?= cclang('add_new_button', [cclang('tb_pegawai_master')]); ?>  (Ctrl+a)" href="<?= admin_site_url('/tb_master_pegawai/add'); ?>"><i class="fa fa-plus-square-o"></i> <?= cclang('add_new_button', [cclang('Master Pegawai')]); ?></a>
                        <?php }) ?>
                     ...
                     <!-- /.widget-user-image -->
                     <h3 class="widget-user-username"><?= cclang('Master Pegawai') ?></h3>
                     <h5 class="widget-user-desc"><?= cclang('list_all', [cclang('Master Pegawai')]); ?> <i class="label bg-yellow"><span class="total-rows"><?= $tb_pegawai_master_counts; ?></span> <?= cclang('items'); ?></i></h5>
                  </div>

                  <form name="form_tb_pegawai_master" id="form_tb_pegawai_master" action="<?= admin_base_url('/tb_master_pegawai/index'); ?>">
                     ...
                     <div class="row">
                        <div class="col-md-8">
                           ...
                           <div class="col-sm-1 padd-left-0 ">
                              <a class="btn btn-default btn-flat" name="reset" id="reset" value="Apply" href="<?= admin_base_url('/tb_master_pegawai'); ?>" title="<?= cclang('reset_filter'); ?>">
...
<script>
   var module_name = "tb_master_pegawai";
...
<script>
   $(document).ready(function() {
            ...
            if (use_ajax_crud == false) {
               ...
               $(document).on('click', '#apply', function() {
                  ...
                  if (bulk.val() == 'delete') {
                     swal({
                        ...
                        function(isConfirm) {
                           if (isConfirm) {
                              document.location.href = ADMIN_BASE_URL + '/tb_master_pegawai/delete?' + serialize_bulk;
                           }
                        });
               ...
               initSortableAjax('tb_master_pegawai', $('table.dataTable'));
            }); /*end doc ready*/
</script>