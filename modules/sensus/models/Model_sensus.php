...
class Model_sensus extends MY_Model
{
    ...
    public function get($q = null, $field = null, $limit = 0, $offset = 0, $select_field = [])
    {
        ...
        $field = in_array($field, $this->field_search) ? $field : "";
        $where = "tb_master_transaksi.tipe_transaksi::text = '3'";
    ...
    {
        $this->db->select("a.*, b.status as status_aset, c.kondisi as kondisi_aset, CASE WHEN a.flag_transaksi = 1 THEN 'Normal' ELSE 'Anomali' END as ceklis_sensus, e.kategori as kategori_aset, EXTRACT(YEAR FROM d.tgl_perolehan) as tahun_perolehan, d.nilai_perolehan, f.ruangan as lokasi_sensus");
        $this->db->from("tb_detail_transaksi a");
        $this->db->join("tb_master_status b", "a.status = b.id", "LEFT");
        $this->db->join("tb_master_kondisi c", "a.id_kondisi = c.id", "LEFT");
    ...
    {
        $this->db->select("a.*, EXTRACT(YEAR FROM b.tgl_perolehan) as tahun_perolehan, c.ruangan AS lokasi_master, d.ruangan AS lokasi_sensus, CASE WHEN a.flag_transaksi = 1 THEN 'Normal' ELSE 'Anomali' END AS status_sensus, e.status AS catatan_sensus, f.kondisi AS kondisi_aset, g.kategori AS kategori_aset");
        $this->db->from("tb_detail_transaksi a");
