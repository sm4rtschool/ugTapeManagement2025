<?php foreach($penggantian_tags as $tb_master_transaksi): ?>
    <tr>
                <td width="5">
        ...
        <td width="200" style="text-align: center">
            ...
            <?php is_allowed('penggantian_tag_view', function() use ($tb_master_transaksi){?>
                <a href="<?= admin_site_url('/penggantian_tag/view/' . $tb_master_transaksi->id); ?>" data-id="<?= $tb_master_transaksi->id ?>" class="label-default btn-act-view"><i class="fa fa-newspaper-o"></i> <?= cclang('view_button'); ?>
            <?php }) ?>
            
            <!-- <?php is_allowed('tb_master_transaksi_update', function() use ($tb_master_transaksi){?>
            <a href="<?= admin_site_url('/penggantian_tag/edit/' . $tb_master_transaksi->id); ?>" data-id="<?= $tb_master_transaksi->id ?>" class="label-default btn-act-edit"><i class="fa fa-edit "></i> <?= cclang('update_button'); ?></a>
            <?php }) ?> -->
            <!-- <?php is_allowed('tb_master_transaksi_delete', function() use ($tb_master_transaksi){?>
                <a href="javascript:void(0);" data-href="<?= admin_site_url('/penggantian_tag/delete/' . $tb_master_transaksi->id); ?>" class="label-default remove-data"><i class="fa fa-close"></i> <?= cclang('remove_button'); ?></a>
            <?php }) ?> -->

        </td>    
    </tr>
<?php endforeach; ?>
<?php if ($penggantian_tag_counts == 0) :?>
        <tr>
        <td colspan="100">
        Penggantian Tag data is not available
        </td>
    </tr>
<?php endif; ?>