...
/**
 *| --------------------------------------------------------------------------
 *| Registrasi People Controller
 *| --------------------------------------------------------------------------
 *| Registrasi People site
 *|
 */
class registrasi_people extends Admin
{

	public function __construct()
	{
		parent::__construct();
		$this->load->model('registrasi_people/model_tb_master_aset', 'model_tb_master_aset');
		$this->load->model('model_registrasi_people');
	...
	{
		$this->is_allowed('registrasi_people_list');

		$filter = $this->input->get('q');
		$field 	= $this->input->get('f');

		$this->data['registrasi_peoples'] = $this->model_registrasi_people->get($filter, $field, $this->limit_page, $offset);
		$this->data['registrasi_people_counts'] = $this->model_registrasi_people->count_all($filter, $field);

		$config = [
			'base_url'     => ADMIN_NAMESPACE_URL  . '/registrasi_people/index/',
			'total_rows'   => $this->data['registrasi_people_counts'],
		...
		$this->data['tables'] = $this->load->view('backend/standart/administrator/registrasi_people/tb_master_transaksi_data_table', $this->data, true);

		if ($this->input->get('ajax')) {
			$this->response([
				'tables' => $this->data['tables'],
				'pagination' => $this->data['pagination'],
				'total_row' => $this->data['registrasi_people_counts']
			]);
		}
	...
	public function serverSideData()
	{
		...
		$totalData = $this->model_registrasi_people->count_all_content();
		$totalFiltered = $totalData;

		if (empty($this->input->post('search')['value'])) {
			$contents = $this->model_registrasi_people->get_content($limit, $start, $order, $dir);
		} else {
			$search = $this->input->post('search')['value'];
			$contents =  $this->model_registrasi_people->content_search($limit, $start, $search, $order, $dir);
			$totalFiltered = $this->model_registrasi_people->content_search_count($search);
		}
	...
	public function add()
	{
		...
		$this->is_allowed('registrasi_people_add');
	...
	public function add_save()
	{
		...
		if (!$this->is_allowed('registrasi_people_add', false)) {
			echo json_encode([
				'success' => false,
		...
		if ($this->form_validation->run()) {
			...
			$save_register_aset = $id = $this->model_registrasi_people->saveRegisterAset($save_data_master_transaksi, $save_data_detail_transaksi, $linked_data);
			...
			if ($save_register_aset) {
				...
				if ($this->input->post('save_type') == 'stay') {
					...
					$this->data['message'] = cclang('success_save_data_stay', [admin_anchor('/registrasi_people', 'Go back to list')]);
				} else {
					set_message(cclang('success_save_data_redirect', [admin_anchor('/registrasi_people/view/' . $save_register_aset, 'See detail')]), 'success');
					$this->data['success'] = true;
					$this->data['redirect'] = admin_base_url('/registrasi_people');
				}
			} else {
				if ($this->input->post('save_type') == 'stay') {
	...
	{
		$this->is_allowed('registrasi_people_update');

		$this->data['registrasi_people'] = $this->model_registrasi_people->find($id);

		$this->template->title('Register Aset Update');
		$this->render('backend/standart/administrator/registrasi_people/registrasi_people_update', $this->data);
	}
	...
	{
		if (!$this->is_allowed('registrasi_people_update', false)) {
			echo json_encode([
				'success' => false,
				'message' => cclang('sorry_you_do_not_have_permission_to_access')
			]);
		...
		if ($this->form_validation->run()) {
			...
			$save_tb_master_transaksi = $this->model_registrasi_people->change($id, $save_data);
			...
			if ($save_tb_master_transaksi) {
				...
				if ($this->input->post('save_type') == 'stay') {
					...
					$this->data['message'] = cclang('success_update_data_stay', [
						admin_anchor('/registrasi_people', ' Go back to list')
					]);
				} else {
					set_message(
	...
	{
		$this->is_allowed('registrasi_people_delete');
		...
		if ($this->input->get('ajax')) {
			if ($remove) {
				$this->response([
					"success" => true,
					"message" => cclang('has_been_deleted', 'registrasi_people')
				]);
			} else {
				$this->response([
					"success" => true,
					"message" => cclang('error_delete', 'registrasi_people')
				]);
			}
		} else {
			if ($remove) {
				set_message(cclang('has_been_deleted', 'registrasi_people'), 'success');
			} else {
				set_message(cclang('error_delete', 'registrasi_people'), 'error');
			}
			redirect_back();
		}
	}
	...
	{
		$this->is_allowed('registrasi_people_view');

		$this->data['tb_master_transaksi'] = $this->model_registrasi_people->getTransaksiById($id);
		$this->data['tb_detail_transaksi'] = $this->model_registrasi_people->getDetailTransaksiById($id);
		$this->template->title('Detail Register Aset');
		$this->render('backend/standart/administrator/registrasi_people/registrasi_people_view', $this->data);
	}
	...
	{
		$tb_master_transaksi = $this->model_registrasi_people->find($id);
		return $this->model_registrasi_people->remove($id);
	}
	...
	{
		$this->is_allowed('registrasi_people_export');

		$this->model_registrasi_people->export(
			'registrasi_people',
			'registrasi_people',
			$this->model_registrasi_people->field_search
		);
	}
	...
	public function export_pdf()
	{
		...
		$this->model_tb_master_transaksi->pdf('registrasi_people', 'registrasi_people');
	}
	...
	{
		if (!$this->is_allowed('registrasi_people_list', false)) {
			echo json_encode([
				'success' => false,
				'message' => cclang('sorry_you_do_not_have_permission_to_access')
			]);
